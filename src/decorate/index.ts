...
import parseSessionMiddleware, {
  SessionOptions,
} from "../middleware/parse-session"
import rateLimitMiddleware, {RateLimitOptions} from "../middleware/rate-limit"
import requireAuthorizationMiddleware, {
  Credentials,
...
export function validateBody(options: ValidationOptions & BodyOptions) {
  return use(
    bufferBodyMiddleware(),
    parseBodyMiddleware(options),
    validateBodyMiddleware(options),
  )
}
